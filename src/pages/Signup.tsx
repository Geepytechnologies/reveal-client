...
const SignUp = () => {
  ...
  const confirmEmail = async () => {
    await fetch(url)
      ...
      .then((data) => {
        setEmaildata(data.deliverability);
      });
  };
  ...
  return (
    <Container>
      ...
      <Wrapper className="mybackground">
        ...
        <p>to Reveal</p>
        <form
          className="w-[100%] mt-[9px] flex flex-col"
          onSubmit={handleLogin}
        >
          <div className="border w-[100%] rounded-xl border-white p-[3px]">
            <Input
              type="text"
              required
          ...
          {PasswordError && (
            <span className="text-[12px] text-center">{passwordErrMsg}</span>
          )}
          {!loading ? (
        ...
        <div
          className="cursor-pointer rounded-xl mt-[15px] bg-[white] text-[#999] w-[90%] flex items-center justify-center"
