...
const Profile = (props: Props) => {
    ...
    const getVideos = async () => {
      try {
        ...
        const res = await axios.get(
          `${domain}/api/users/find/videos/${currentuser?.others._id}`
        );
  ...
  const deletevideo = async () => {
    await axios.delete(`${domain}/api/videos/${videoid}`, {
  ...
  return (
    <>
      ...
      >
        ...
        <DialogActions>
          <button
            className="bg-[var(--secondary)] text-white py-[8px] rounded-xl px-[15px]"
            onClick={deletevideo}
          >
            Yes
          </button>
      ...
      <div className=" flex flex-col  pb-[50px] ">
        <div className="w-[100%] blur-[2px] bg-cover  border-white min-h-[300px] bg-[url('/cinematicview.png')] bg-center "></div>
        <div className="flex gap-3 flex-col z-[50] pl-[10px] mt-[-40px] w-[80%]">
          <div className="flex flex-col">
            <Userimg
              className="h-[150px] border-2 border-white rounded-full w-[150px]"
              src={currentuser?.others.img}
              alt="Profile"
            />
            <Username className="font-[600]">
              {currentuser?.others.username}
            </Username>
          </div>
          <Text>{currentuser?.others.email}</Text>
          <Subs>
            <SubscriptionsIcon
              className={darkMode ? "text-white" : "text-black"}
            />
            <SubsText>{currentuser?.others.subscribers} subscribers</SubsText>
          </Subs>
          ...
          <div>
            ...
            <div className="flex gap-3 flex-col">
              {!videos?.length && (
                <Text>You don&apos;t have any Video yet</Text>
              )}
                        ...
                        <div className="flex flex-col justify-between">
                          ...
                          <div className="flex">
                            <VisibilityIcon
                              className={darkMode ? "text-white" : "text-black"}
                            />
                            <Text className="text-[14px] ml-[5px]">
                              {video?.views} Views
                            </Text>
                          </div>
                          <div className="flex items-center mt-[10px]">
                            <DeleteIcon
                              ...
                              className="text-[var(--secondary)]  mr-[5px]"
                            />
                            <EditIcon
          ...
          <Logout
            className="rounded-lg w-[200px] px-[5px] py-[10px]"
            onClick={handleClick}
          >
            Logout
          </Logout>
